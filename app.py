...
#     predicted_class_name, confidence_item, cam_output_path = classify(file_path)

#     session["disease"] = predicted_class_name
#     session["percentage"] = confidence_item * 100
#     session["cam_image"] = cam_output_path
...
#     disease = session.get("disease")
#     percentage = session.get("percentage")
#     cam_image = session.get("cam_image")

#     return render_template("results.html", disease=disease, percentage=percentage, cam_image=cam_image)
...
def upload():
    ...
    os.rename(cam_output_path, "static\\result.jpeg")
    cam_output_path = "result.jpeg"
    session["disease"] = predicted_class_name
    session["percentage"] = confidence_item * 100
    session["cam_image"] =  cam_output_path.replace('\\', '/')  # Ensure correct path

    print(f"CAM Image Path: {session['cam_image']}")  # Debugging statement
